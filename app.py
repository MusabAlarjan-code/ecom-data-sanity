...
if uploaded_file is not None:
    ...
    quantities[np.random.choice(rows, 10, replace=False)] = -5 # قيم سالبة خاطئة
    quantities[np.random.choice(rows, 8, replace=False)] = np.nan # قيم مفقودة
    
    prices = np.random.choice([150, 300, 45, 1200], size=rows).astype(float)
    prices[np.random.choice(rows, 5, replace=False)] = np.nan # أسعار مفقودة
...
if df_raw is not None:
    ...
    # إصلاح الكميات والأسعار المفقودة والسالبة
    ...
    # حساب إجمالي المبيعات الصحيح برمجياً
    ...
    # حساب تشتت وتذبذب المبيعات (المعامل الإحصائي CV)
    cv_sales = df_clean['إجمالي_المبيعات'].std() / avg_order
    
    # المبيعات حسب المدن وحسب المنتجات
    ...
    # لوحة قياس الأرقام التجارية الرئيسية
    ...
    # توليد التوصيات والتحليلات المؤتمتة ذكياً
    st.markdown("### 🧠 التوصيات وقراءة الأداء المالي للفترة الحالية:")
    
    insight_text = f"• **المنتج القيادي:** يعتبر منتج (**{top_product}**) هو المحرك الأساسي لحجم الإيرادات في المتجر، نوصي بتركيز الميزانيات التسويقية عليه لضمان استمرار التدفق النقدي.\n\n"
    insight_text += f"• **التوزيع الجغرافي:** تعد مدينة (**{top_city}**) هي السوق الأكثر نشاطاً وتوليداً للأرباح خلال هذه الفترة.\n\n"
    
    if cv_sales > 0.8:
        insight_text += "• **تنبيه إدارة المخاطر (تذبذب عالٍ):** يلاحظ النظام وجود تشت
